...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_batch_integration/task_batch_integration/src/methods/era_combat_bbknn/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "target/nextflow/methods/era_combat_bbknn",
    "viash_version" : "0.9.4",
...
"""ERA ComBat BBKNN code for removing batch effects.

This is the top-performing batch correction implementation discovered by the AI
system described in https://www.nature.com/articles/s41586-026-10658-6 . In that
article this implementation is referred to as "BBKNN (TS)" but has been renamed
to "ERA ComBat BBKNN" here to provide more clarity on its origin.
"""
## VIASH START
...
par = {
  ...
  'n_pca_components': $( if [ ! -z ${VIASH_PAR_N_PCA_COMPONENTS+x} ]; then echo "int(r'${VIASH_PAR_N_PCA_COMPONENTS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'n_neighbors_per_batch': $( if [ ! -z ${VIASH_PAR_N_NEIGHBORS_PER_BATCH+x} ]; then echo "int(r'${VIASH_PAR_N_NEIGHBORS_PER_BATCH//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'total_k_neighbors': $( if [ ! -z ${VIASH_PAR_TOTAL_K_NEIGHBORS+x} ]; then echo "int(r'${VIASH_PAR_TOTAL_K_NEIGHBORS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi )
}
meta = {
...
global_config = {
    k: par[k]
    for k in ('n_pca_components', 'n_neighbors_per_batch', 'total_k_neighbors')
}
heavy_output = eliminate_batch_effect_fn(input_adata, config=global_config)

# Save only the outputs relevant for scoring.
output = ad.AnnData(obsm={'X_emb': heavy_output.obsm['X_emb']},
                    obsp=heavy_output.obsp,
                    uns=heavy_output.uns)
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_batch_integration/methods/era_combat_bbknn",
    "tag" : "build_main"
  },
  "label" : [
    "midtime",
