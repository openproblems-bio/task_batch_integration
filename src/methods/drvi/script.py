...
if not np.allclose(counts, np.round(counts)):
    warnings.warn(
        "Non-integer values detected in 'counts' layer. "
        "DRVI expects count data. Rounding to nearest integers as a workaround."
    )
    adata.layers["counts"] = np.round(counts).astype(int)
